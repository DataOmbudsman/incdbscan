...
def test_new_object_far_from_cluster_is_labeled_as_noise(
    incdbscan4, blob_in_middle, object_far_away
):
...
def test_new_border_object_gets_label_from_core(incdbscan4):
    ...
    assert_two_objects_are_in_same_cluster(incdbscan4, cluster[[0]], new_border_object)


def test_labels_are_noise_only_until_not_enough_objects_in_cluster(
    incdbscan4, blob_in_middle
):
...
def test_two_clusters_can_be_born_at_the_same_time(incdbscan4, point_at_origin):
    ...
    assert_label_of_object_is_among_possible_ones(
        incdbscan4, new_object, {cluster_1_label_expected, cluster_2_label_expected}
    )
...
def test_absorption_with_noise(incdbscan3, point_at_origin):
    ...
    insert_objects_then_assert_cluster_labels(incdbscan3, noise, CLUSTER_LABEL_NOISE)
...
def test_merge_two_clusters(incdbscan3, point_at_origin):
    ...
    new_object = point_at_origin
    merged_cluster_expected_label = max(
        [cluster_1_expected_label, cluster_2_expected_label]
    )
...
def test_merger_and_creation_can_happen_at_the_same_time(
    ...
    assert_cluster_labels(incdbscan4, bottom_right, bottom_right_expected_label)
    ...
    merged_cluster_expected_label = incdbscan4.get_cluster_labels(bridge_point)[0]
    ...
    insert_objects_then_assert_cluster_labels(
        incdbscan4, left_pre_cluster, CLUSTER_LABEL_NOISE
    )
    ...
    assert_cluster_labels(incdbscan4, top_right, merged_cluster_expected_label)
    assert_cluster_labels(incdbscan4, bottom_right, merged_cluster_expected_label)
    assert_cluster_labels(incdbscan4, bridge_point, merged_cluster_expected_label)
    assert_cluster_labels(incdbscan4, left_pre_cluster, left_cluster_expected_label)
...
def test_two_mergers_can_happen_at_the_same_time(
    ...
    assert_cluster_labels(
        incdbscan4, np.vstack([top_right, bottom_right]), bottom_right_expected_label
    )

    assert_cluster_labels(
        incdbscan4, np.vstack([top_left, bottom_left]), bottom_left_expected_label
    )
...
def test_object_is_core_if_it_has_more_than_enough_neigbhors(
    ...
    incdbscan3.insert(neighbors)
    incdbscan3.insert(point_at_origin)

    assert_cluster_labels(incdbscan3, neighbors, expected_label)
    assert_cluster_labels(incdbscan3, point_at_origin, expected_label)
    assert_label_contains_objects(
        incdbscan3, expected_label, np.vstack((neighbors, point_at_origin))
    )
