...
from src.incrementaldbscan.incrementaldbscan import IncrementalDBSCANWarning


def test_no_warning_raised_if_unknown_id_is_passed_to_add(
        incdbscan4,
        blob_in_middle):
    ...
    with pytest.warns(None) as record:
        incdbscan4.add_objects(object_values, object_ids)

    number_of_warnings = len(record)
    assert number_of_warnings == 0


def test_warning_raised_if_known_id_is_passed_to_add(
        incdbscan4,
        blob_in_middle):
    ...
    with pytest.warns(IncrementalDBSCANWarning):
        incdbscan4.add_objects([object_values[0]], [object_ids[0]])
...
def test_labels_are_accessible_for_added_objects(incdbscan4, blob_in_middle):
    object_values, object_ids = blob_in_middle
    incdbscan4.add_objects(object_values, object_ids)
...
def test_labels_are_not_accessible_for_not_added_objects(
        incdbscan4,
        blob_in_middle):
